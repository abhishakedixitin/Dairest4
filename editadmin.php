...
if(!isset($editid)||empty($editid)){
    header("location:check_user_table.php");
}

$sql = "SELECT * FROM users WHERE ID Like '{$editid}'";
...
if($_POST){
    ...
    $sql2 = "UPDATE users SET  username = '{$username}', password='{$password}', u_mobile='{$mobile}', u_email='{$email}' where id Like '{$id}'";
    $result2 = $conn->query($sql2) or die(mysqli_error());

    if($result2){
        echo "<script> alert('Record Updated');window.location='check_user_table.php' ;</script>";
    }
}
?>
